...
def track_objects_in_video(
    ...
    print("\n" + "="*60)
    print(" [Step 2] 비디오 트래킹 (Tracking) 및 JSON 저장 시작 ")
    print("="*60)
    ...
    for i in range(num_detected):
        ...
        # [수정] add_new_mask는 (H, W) 형태의 2D Tensor를 요구함
        if mask_input.dim() == 3:
    ...
    for frame_idx, obj_ids, low_res_masks, video_res_masks, obj_scores in predictor.propagate_in_video(
        ...
        if frame_idx % 100 == 0:
            ...
            cutoff = frame_idx - 50
            if cutoff > 0:
                # [중요] cond_frame_outputs(0번)는 남기고 non_cond만 삭제
        ...
        if frame_img is not None and video_res_masks is not None and len(video_res_masks) > 0:
            ...
            # 결과 리스트에 추가
            tracking_data.append(frame_results)
            # 이미지 저장
    ...
    print(f"[Saving] JSON 저장 중... {json_path}")
    try:
        with open(json_path, 'w') as f:
            json.dump(tracking_data, f)
        print("[Success] JSON 저장 완료.")
    except Exception as e:
        print(f"[Error] JSON 저장 실패: {e}")
        import pickle
        pkl_path = os.path.join(output_dir, "tracking_results.pkl")
        with open(pkl_path, 'wb') as f:
            pickle.dump(tracking_data, f)
        print("[Backup] Pickle 파일로 백업되었습니다.")
