...
# ==============================================================================
# [Part 1] 객체 검출 (수정됨: 특정 프레임 지정 가능)
# ==============================================================================
def detect_objects(frame_dir: str, text_prompt: str, target_frame_idx: int = 0, model_path: str = None) -> Dict[str, Any]:
...
def run_bidirectional_tracking(frame_dir: str, detection_results: Dict, json_output_dir: str, start_frame_idx: int):
    ...
    for frame_idx, obj_ids, _, video_res_masks, _ in predictor.propagate_in_video(
        ...
        # [VRAM 관리: 정방향] 현재보다 7프레임 이상 과거의 데이터 삭제
        if frame_idx > start_frame_idx and frame_idx % 100 == 0:
            cutoff = frame_idx - 7
            if cutoff > start_frame_idx: # 시작 프레임보다는 커야 함 (시작 정보 보존)
    ...
    # --------------------------------------------------------------------------
    # 3. 중간 메모리 정리 (중요)
    # 역방향 가기 전에, 방금 계산한 정방향 추론 데이터(non_cond)는 이미 저장했으니 삭제
    # 단, start_frame_idx(조건 프레임)는 남겨둬야 함.
    # --------------------------------------------------------------------------
    print("🧹 방향 전환 전 VRAM 정리 중...")
    outputs = state["output_dict"]["non_cond_frame_outputs"]
    # start_frame_idx보다 큰 모든 추론 결과 삭제
    keys_to_remove = [k for k in outputs.keys() if k > start_frame_idx]
    for k in keys_to_remove: del outputs[k]
    torch.cuda.empty_cache()

    # --------------------------------------------------------------------------
    # 4. 역방향 추적 (Start Frame -> 0)
    # --------------------------------------------------------------------------
    print(f"⬅️ 역방향(Backward) 추적 진행중... ({start_frame_idx} -> 0)")
    for frame_idx, obj_ids, _, video_res_masks, _ in predictor.propagate_in_video(
        state, start_frame_idx=start_frame_idx, max_frame_num_to_track=None, reverse=True, propagate_preflight=True
    ):
        save_frame_json(frame_idx, obj_ids, video_res_masks, state, json_output_dir)
        
        # [VRAM 관리: 역방향] 현재보다 7프레임 이상 '미래'(원래 진행방향의 과거) 데이터 삭제
        # 역방향이므로 frame_idx가 점점 작아짐 (예: 900 -> 800)
        # 따라서 frame_idx + 7 보다 큰(더 먼 미래) 데이터를 지워야 함
        if frame_idx < start_frame_idx and frame_idx % 100 == 0:
            cutoff = frame_idx + 7
            if cutoff < start_frame_idx:
                outputs = state["output_dict"]["non_cond_frame_outputs"]
                # 여기서는 cutoff보다 큰 키를 지워야 함 (이미 처리하고 지나온 프레임들)
...
def main():
    ...
    json_output_dir = "/workspace/nas203/ds_RehabilitationMedicineData/IDs/tojihoo/data/data/9_KEYPOINTS_V2/AI_dataset/N01/N01_Treatment/diagonal__biceps_curl"
    prompt = "person"
    
    # ⭐ [중요] 시작할 프레임 번호 (이 프레임에서 객체를 찾고 앞뒤로 퍼짐)
    START_FRAME_IDX = 1860  # 예: 400번 프레임에서 시작
    # ▲ 경로 설정 ▲
    ...
    detection_res = detect_objects(frame_dir, prompt, target_frame_idx=START_FRAME_IDX)
    
    # 2. 양방향 Tracking
    if detection_res:
        run_bidirectional_tracking(frame_dir, detection_res, json_output_dir, start_frame_idx=START_FRAME_IDX)
